...
ipip <- Df %>% 
  select(responseid, starts_with("ipip")) 
...
ipip <- ipip %>% 
  transmute(responseid = responseid,
            ...
            ipip19_r = 6 - ipip19,
            ipip20_r = 6 - ipip20,) %>%
mutate(ipip_tot = rowSums(select(., c(2:21))))
...
Stai <- Df %>% 
  select(responseid, starts_with("Stai")) 
...
Stai <- Stai %>% 
  transmute(responseid = responseid,
            ...
            Stai19_r = 5 - stai19,
            Stai20_r = 5 - stai20,) %>%
  mutate(Stai_tot = rowSums(select(., c(2:21))))
...
sam_emotional <- Df %>% 
  select(responseid, starts_with("Sam1")) 

sam_arousal <- Df %>% 
  select(responseid, starts_with("Sam2")) 

sam_control <- Df %>% 
  select(responseid, starts_with("Sam3")) 
